...
async function openMailtoWithLink(
  ...
  try {
    ...
    return {
      success: clipboardOk,
      message: clipboardOk ? `הקישור הועתק ללוח – שלח ידנית ל-${toEmail}` : `לא ניתן לפתוח מייל. שלח ידנית ל-${toEmail}`,
    }
  }
  return {
    success: true,
    message: clipboardOk ? `✉️ תוכנת המייל נפתחה! הקישור הועתק ללוח` : `✉️ תוכנת המייל נפתחה לשליחה ל-${toName} (${toEmail})`,
  }
}
...
export async function sendApprovalRequestEmail(
  ...
  try {
    ...
    await emailjs.send(SERVICE_ID, TEMPLATE_APPROVAL_ID, {
...
export async function sendHrFinalEmail(
  ...
  const formLink = encodeFormToUrl(form)
  const approversChain = form.approvalSteps.map((s) => `${s.managerName} (${s.title})`).join(' → ')
  ...
  try {
    // @ts-ignore
    const emailjs = await import('@emailjs/browser')
    await emailjs.send(SERVICE_ID, TEMPLATE_HR_FINAL_ID, {
      to_email: toEmail,
    ...
    console.error('שגיאה בשליחת מייל HR:', err)
    return openMailtoWithLink(toEmail, 'מחלקת משאבי אנוש', subject, formLink)
  }
}
